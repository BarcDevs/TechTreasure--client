    ...
    return (
        <Form {...form}>
            <form
                ...
                className="max-w-[370px] flex-col gap-10 space-y-8">
                <div className={'flex-col-start gap-6'}>
                    <h2 className={'text-big-medium'}>
                        {t(AUTH_LOCALES.headlineSignup)}
                    </h2>
                    <p className={'text-body'}>
                        {t(AUTH_LOCALES.subheadingSignup)}
                    </p>
                </div>
                <FormInput
                    name={'name'}
                    placeholder={t(AUTH_LOCALES.name)}
                    className={inputStyle}
                    formControl={form.control}
                />
                <FormInput
                    name={'email'}
                    placeholder={t(AUTH_LOCALES.email)}
                    type={'email'}
                    className={inputStyle}
                    formControl={form.control}
                />
                <FormInput
                    name={'password'}
                    placeholder={t(AUTH_LOCALES.password)}
                    type={'password'}
                    className={inputStyle}
                    formControl={form.control}
                />
                <FormInput
                    ...
                    placeholder={t(AUTH_LOCALES.confirmPassword)}
                    type={'password'}
                    className={inputStyle}
                    formControl={form.control}/>
                {/*<div className="flex items-center gap-2">*/}
                {/*    <Checkbox id="admin-checkbox"*/}
                {/*              checked={isAdmin}*/}
                ...
                <div className={'flex_col gap-4'}>
                    <Button className={'w-full capitalize'}
                            type="submit"
                            text={t(AUTH_LOCALES.createAccount)}
                    />
                ...
                <div className={'flex-center-row gap-4'}>
                    <p className={'text-body'}>
                        {t(AUTH_LOCALES.alreadyHaveAccount)}
                    </p>
                    <Link to={'/login'}
                          className={'text-body-medium underline'}>
                        {t(AUTH_LOCALES.login)}
                    </Link>
                </div>
            </form>
        </Form>
